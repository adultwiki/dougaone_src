 ...
 */
(function ($, CONFIG) {
    var $main = $('.ins-search');                           //フォーム全般
    var $input = $main.find('.ins-search-input');　         //検索文字列
    ...
    $(document).on('click', '.search-button', function () {
       // alert()
        //PC用の検索ボックスに入力あり　かつ　ウィンドウサイズが479より大きい場合、押下されたら検索フォームをサブミット
        if($(".search-text-pc").val() !="" && $(window).width() > 479){        
           ...
           // $("#search-category").val("chinko")

            //window.location.href = "/search/search.html" + "?s=" +$(".search-text-pc").val()


            // //検索テキストと検索押下時のカテゴリ名をくっつけて検索画面に遷移
            // if(currentPageName != 'ホーム'){
            //     window.location.href = "/search/search.html" + "?s=" +$(".search-text-pc").val()　+ "&c=" + currentPageName;
            // }else{
            //     window.location.href = "/search/search.html" + "?s=" +$(".search-text-pc").val();
            // }            
        }        

        //ウィンドウサイズが479以下（mobile）の場合、検索テキスト入力欄を表示・非表示
        if($(window).width() <= 479){
    ...
    //画面幅４７９より大きい（PC）場合、モバイル用検索欄を非表示
    $(window).resize(hideSearchBox)
